...
void FMineSweeperStyle::Initialize()
{
	if (!StyleInstance.IsValid())
	{
		...
		StyleInstance->Set("MineSweeper.GrayBox", new FSlateRoundedBoxBrush(FLinearColor::Gray, 3.f));
		StyleInstance->Set("MineSweeper.RedBox", new FSlateRoundedBoxBrush(FLinearColor::Red, 3.f));
		StyleInstance->Set("MineSweeper.BlackBox", new FSlateRoundedBoxBrush(FLinearColor::Black, 3.f));
	}
}
